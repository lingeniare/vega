...
export const models: Model[] = [
  ...
  // Lite Models (continued)
  {
    ...
    pro: false,
    ultra: false,
    requiresAuth: false, // Изменено: доступна для неавторизованных пользователей
    freeUnlimited: false,
...
export function getAcceptedFileTypes(modelValue: string, isProUser: boolean, isUltraUser: boolean = false): string {
  const model = getModelConfig(modelValue);
  if (model?.pdf && (isProUser || isUltraUser)) {
    return 'image/*,.pdf';
  }
  return 'image/*';
}
